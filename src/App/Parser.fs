...
module Lexer =
  ...
  *)
    ...
    let private lerrat kind (l: lexer) msg = errat kind (l.name, l.line, l.char) msg

    let close (l: lexer) =
        match l.buffer with
        | Some _ -> lerrat "lexical error" l "input not exhaused"
        | _ ->
            match l.istream.Peek() with
            | -1 -> l.istream.Close()
    ...
    let rec private readtoken (l: lexer) =
        ...
        | Some c ->
            ...
            then (p1, readname l [ c ])
            else if System.Char.IsDigit c
            then (p1, readnumber l [ c ])
            else lerrat "lexical error" l (sprintf "unexpected character: %c" c)
...
module FixityParser =
    ...
    let rec parse jux next =
        ...
        and parse_prefix' st ps ch next =
            ...
            | (pos, _, None, Some i) -> errat "parse error" pos "infix operator in prefix position"
        ...
        and parse_infix' st ps E ch next =
            ...
            | (st, (pr', make') :: ps, (pos, _, _, Some ((pr, a, make) as i))) ->
                ...
                then parse_prefix (FRAME(st, (pr', make') :: ps, E, i)) [] next
                else errat "parse error" pos "precedence conflict"
            | (st, (pr', make') :: ps, (pos, _, _, None)) ->
                ...
                then parse_prefix' (FRAME(st, (pr', make') :: ps, E, jux)) [] ch next
                else errat "parse error" pos "precedence conflict"
...
let private expect (l, t) =
    ...
    if t' = t
    then ()
    else errat "parse error" p' (sprintf "expected token %s, got %s" (Lexer.show t) (Lexer.show t'))
...
let rec private chunkifyExp (l: Lexer.lexer) () =
    ...
    | (p, (Lexer.TPUNC ',' as t)) ->
        Lexer.unread l t
        None
    | (p, (Lexer.TNAME "in" as t)) ->
        Lexer.unread l t
        None
    | (p, (Lexer.TNAME "then" as t)) ->
        Lexer.unread l t
        None
    | (p, (Lexer.TNAME "else" as t)) ->
        Lexer.unread l t
        None
