...
let execProg prog st =
    let rec exec ins st =
        ...
        | (IEQ :: ins, y :: x :: st) -> if x = y then exec ins (1 :: st) else exec ins (0 :: st)
        | (ILT :: ins, y :: x :: st) -> if x < y then exec ins (1 :: st) else exec ins (0 :: st)
        | (ILE :: ins, y :: x :: st) -> if x <= y then exec ins (1 :: st) else exec ins (0 :: st)
